...
// Użytkownicy: najpierw adresy, potem użytkownicy z AddressId (używamy UserFaker.Generate(IEnumerable<int>))
...
foreach (var user in users)
{
    // Klienci: wygeneruj 25 adresów i 25 klientów, a następnie przypisz AddressId do każdego klienta
...
foreach (var user in users)
{
    ...
    // newClient2 z własnym adresem (invoice with new client snapshot)
    ...
    // nowe produkty i kolejna faktura z existingClient

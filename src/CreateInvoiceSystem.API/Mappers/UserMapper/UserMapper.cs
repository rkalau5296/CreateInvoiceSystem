...
internal static class UserMapper
{
    ...
    {
        if (e == null) return null!;
        var positions = allPositions?.Where(ip => ip.InvoiceId == e.InvoiceId).Select(MapInvoicePosition).ToList() ?? [];
        return new Invoice
        {
            InvoiceId = e.InvoiceId,
            Title = e.Title,
        ...
        IEnumerable<AddressEntity> clientAddresses)
    {
        ...
        var mappedInvoices = invoices?.Select(i => MapInvoice(i, invoicePositions)).ToList() ?? [];

        var mappedProducts = products?.Select(MapProduct).ToList() ?? [];
    ...
    public static User MapSummary(UserEntity u, AddressEntity addr, IEnumerable<InvoiceEntity> allInvoices, IEnumerable<InvoicePositionEntity> allInvoicePositions, IEnumerable<ProductEntity> allProducts, IEnumerable<ClientEntity> allClients, IEnumerable<AddressEntity> allClientAddresses)
    {
        ...
        var invoices = allInvoices?.Where(i => i.UserId == u.Id).Select(i => MapInvoice(i, allInvoicePositions)).ToList() ?? [];

        var products = allProducts?.Where(p => p.UserId == u.Id).Select(MapProduct).ToList() ?? [];

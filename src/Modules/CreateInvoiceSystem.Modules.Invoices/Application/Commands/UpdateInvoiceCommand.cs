...
public class UpdateInvoiceCommand : CommandBase<UpdateInvoiceDto, UpdateInvoiceDto>
{
    public override async Task<UpdateInvoiceDto> Execute(IDbContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (this.Parametr.InvoicePositions is not null)
        {
            ...
            {
                context.Set<InvoicePosition>().Remove(invoicePositionToDelete);                
            }
            ...
            foreach (var inovoicePositionToAdd in inovoicePositionsToAdd)
            {
                ...
                await context.Set<InvoicePosition>().AddAsync(newInovicePosition, cancellationToken);
            }
        }
        ...
        var persisted = await context.Set<Invoice>()
            ...
            .SingleAsync(i => i.InvoiceId == invoice.InvoiceId, cancellationToken);
    ...
    private static async Task<Product> GetOrCreateProductAsync(string productName, string productDescription, decimal? productValue, int userId, IDbContext context,   CancellationToken cancellationToken)
    {        
